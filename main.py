...
def search_documents(query: str = Query(...), db: Session = Depends(get_db)):
    # Query by name or id or extracted content
    results = db.query(Document).join(ExtractedText).filter(
        (Document.name.ilike(f"%{query}%")) |                     # search by name
        (ExtractedText.content.ilike(f"%{query}%")) |            # search by content
        (Document.id == query if query.isdigit() else False)     # search by id if number
    ).all()
...
# -------------------- CATCH-ALL FRONTEND ROUTE (MOVE TO END) --------------------
